...
public class DropperFakeInventory extends FakeInventory {
    
    @Getter
    @Setter
    private String name;

    public DropperFakeInventory() {
        this(null);
    }

    public DropperFakeInventory(InventoryHolder holder) {
        this(holder, null);
    }
    ...
    public DropperFakeInventory(InventoryType type, InventoryHolder holder, String title) {
    ...
    protected List<BlockVector3> onOpenBlock(Player who) {
        ...
        placeDropper(who, blockPosition);

        return Collections.singletonList(blockPosition);
    }

    private void placeDropper(Player who, BlockVector3 pos) {
        who.getLevel().sendBlocks(new Player[] { who }, new Vector3[] {});
        UpdateBlockPacket updateBlock = new UpdateBlockPacket();
        updateBlock.blockRuntimeId = BlockStateRegistry.getRuntimeId(BlockState.of(BlockID.DROPPER));
    ...
    private static byte[] getNbt(BlockVector3 pos, String name) {
        CompoundTag tag = new CompoundTag()
                ...
                .putInt("y", pos.y)
                .putInt("z", pos.z)
                .putString("CustomName", name == null ? "Dropper" : name);

        try {
            return NBTIO.write(tag, ByteOrder.LITTLE_ENDIAN, true);
        } catch (IOException e) {
            throw new RuntimeException("Unable to create NBT for dropper");
        }
    }
}
